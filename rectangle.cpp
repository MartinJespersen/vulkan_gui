...
void createRectangleIndexBuffer(Vulkan_Rectangle &vulkanRectangle,
  ...
  createBuffer(
      ...
      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, vulkanRectangle.boxIndexBuffer,
      vulkanRectangle.boxIndexMemoryBuffer);

  copyBuffer(device, commandPool, graphicsQueue, stagingBuffer,
             vulkanRectangle.boxIndexBuffer, bufferSize);

  vkDestroyBuffer(device, stagingBuffer, nullptr);
  vkFreeMemory(device, stagingBufferMemory, nullptr);
}

void createRectangleInstBuffer(Vulkan_Rectangle &vulkanRectangle,
  ...
  createBuffer(physicalDevice, device, bufferSize,
               ...
               vulkanRectangle.boxInstBuffer, vulkanRectangle.boxMemoryBuffer);

  copyBuffer(device, commandPool, graphicsQueue, stagingBuffer,
             vulkanRectangle.boxInstBuffer, bufferSize);

  vkDestroyBuffer(device, stagingBuffer, nullptr);
  vkFreeMemory(device, stagingBufferMemory, nullptr);
}

void beginRectangleRenderPass(
    Vulkan_Rectangle &vulkanRectangle, VkCommandBuffer commandBuffer,
  ...
  vkCmdBindIndexBuffer(commandBuffer, vulkanRectangle.boxIndexBuffer, 0,
                       VK_INDEX_TYPE_UINT16);

  vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                          vulkanRectangle.pipelineLayout, 0, 1, &descriptorSet,
                          0, nullptr);

  vkCmdPushConstants(commandBuffer, vulkanRectangle.pipelineLayout,
                     VK_SHADER_STAGE_VERTEX_BIT, resolution.bufferInfo.offset,
                     resolution.size(), resolution.data);

  vkCmdDrawIndexed(commandBuffer, 6, static_cast<uint32_t>(boxInstances.size()),
                   0, 0, 0);
