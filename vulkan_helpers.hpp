...
struct Vulkan_Resolution {
  ...
  inline uint32_t size() { return sizeof(data); }
};
...
void transitionImageLayout(VkCommandPool commandPool, VkDevice device,
...
void copyBufferToImage(VkCommandPool commandPool, VkDevice device,
...
VkImageView createColorResources(VkPhysicalDevice physicalDevice,
  ...
  createImage(
      physicalDevice, device, swapChainExtent.width, swapChainExtent.height,
...
createFramebuffers(VkDevice device, VkImageView colorImageView,
  ...
  for (size_t i = 0; i < swapChainImageViews.size(); i++) {
    std::array<VkImageView, 2> attachments = {colorImageView,
