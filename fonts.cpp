...
void createGlyphAtlasImage(VkPhysicalDevice physicalDevice, VkDevice device, VkCommandPool commandPool, VkQueue graphicsQueue)
{
    ...
    createGlyphAtlasImage(physicalDevice, device, texWidth, texHeight, VK_SAMPLE_COUNT_1_BIT, VK_FORMAT_R8_UNORM, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, textureImage, textureImageMemory);
...
void createGlyphAtlasColorResources(VkPhysicalDevice physicalDevice, VkDevice device, VkFormat swapChainImageFormat, VkExtent2D swapChainExtent)
{
    VkFormat colorFormat = swapChainImageFormat;

    createGlyphAtlasImage(physicalDevice,
                          device,
                          ...
                          glyphAtlasColorImage,
                          glyphAtlasColorImageMemory);

    glyphAtlasColorImageView = createImageView(device, glyphAtlasColorImage, colorFormat);
}

void cleanupGlyphAtlasSwapChain(VkDevice device)
{
    vkDestroyImageView(device, glyphAtlasColorImageView, nullptr);
    vkDestroyImage(device, glyphAtlasColorImage, nullptr);
    vkFreeMemory(device, glyphAtlasColorImageMemory, nullptr);
}